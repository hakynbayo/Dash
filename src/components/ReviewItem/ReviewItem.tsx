...
const Reviews: React.FC = () => {
  ...
  const handleThumbsUpClick = (event: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
    event.stopPropagation();
    setThumbsUpCount(thumbsUpCount + 1);
  };

  const handleThumbsDownClick = (event: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
    event.stopPropagation();
  ...
  return (
    <div className='w-full mb-6 flex flex-col bg-white h-max px-4  gap-2 cursor-pointer' onClick={toggleCommentVisibility}>
      ...
      <div className='cursor-pointer' onClick={toggleCommentVisibility}>
        <p className='text-base font-normal text-[#18181B]'>
          There is no stable electricity. The roads are fairly
          good and there is a sense of community.
          The drainage system is poor and most residents
          litter their surroundings.
          There are several grocery stores and Supermarkets.
        </p>
      </div>
      ...
      {isCommentVisible && (
        <div className='flex justify-between items-center border-b'>
          ...
          />
          <Button className='bg-blue py-0 rounded-md'>
            Post
          </Button>
        </div>
      )}
